...
"""
Script to regrid FESOM-C output to transects defined by shapefile lines.
Interpolates unstructured data to regular grid along specified transects.
...
path2='./'
infile = path2+'indi0.nc'
outfile = path2+'lines_regular_'# .nc will be added
#lfile = path2+'/work/projects/indiga/mesh/lines_equalpoints.shp'
lfile = '/work/projects/indiga/mesh/maxdepth_river.shp'
path2output = path2
path2png    = path2+'png/'
...
for iline in range(nlines):
    ...
    for itime,time in enumerate(tindw):
        ...
        z_u = d*s #2d array with z levels at each node
        z_t = (z_u[:,0:-1]+z_u[:,1:])/2.0    
